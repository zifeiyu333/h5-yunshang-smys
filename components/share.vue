			...
			<scroll-view class="view-content" scroll-y>
				...
				<view class="share-list">
					<view v-for="(item, index) in shareList" :key="index" class="share-item">
						<button v-if="item.type === 1" class="share-item" open-type="share">
							<image :src="item.icon" mode=""></image>
							<text>{{item.text}}</text>
						</button>
						<view v-if="item.type === 2" class="share-item" @click="shareToFriend(item)">
							<image :src="item.icon" mode=""></image>
							<text>{{item.text}}</text>
						</view>
					</view>
...
<script>
	export default {
		...
		methods:{
			...
			shareToFriend(item){
				if (item.type === 2) {
					this.$emit('shareCallback', item.type);
					this.toggleMask();
				} else {
					this.toggleMask();
				}

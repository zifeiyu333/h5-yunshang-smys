<template>
	...
	<view class="canvas_box" :hidden="canvasFlag">
		...
		<text type="cancel" class="eosfont eos-delete canvas_close_btn" @tap="canvasCancelEvn"></text>
		<!-- <icon type="cancel" class="canvas_close_btn" size="60" @tap="canvasCancelEvn" color="transparent"/> --><!-- 关闭 -->
		<view class="button-wrapper"><!-- 保存海报按钮 -->
			<button class="confirm-btn save_btn" @click="saveCanvasImage">保存相册，分享到朋友圈</button>
		</view>
	</view>
</template>
...
<script>
	export default{
		...
		props:{
			...
			posterObj:{
				url:'',//商品主图
				icon:'https://kaipusi.cn/static/hch-hyj.png',//会员价图标
				title:'',//标题
		...
		methods:{
			...
			*/
			...
			createCanvasImage() {
				...
				let w = (row.length-1) * 10
				let scaleW = this.phoneW/375;//按照苹果留 375*667比例 其他型号手机等比例缩放 显示
				let scaleH = this.phoneH/(590+w);//按照苹果留 375*667比例 其他型号手机等比例缩放 显示
				...
				ctx.save();
				this.roundRect(ctx,50,40,(this.phoneW-100), (325+w)*scaleH,10,'#f7f7f7','#f7f7f7');//绘制海报圆角背景 上半截灰色的
				ctx.restore();
				//将网络图片转成本地路径 商品图片
			    wx.getImageInfo({
			       src: url,
			...
			// 	})
				...
				// 海报商品title
				setTimeout(()=>{
					let contentTextY = 343; // 这段文字起始的y位置
			...
			         src: zpPriceIcon,
			         success(res) {
					ctx.drawImage(res.path,65,(340+w)*scaleH,44, 15)
					ctx.draw(true)
			      } ,
					fail(){
						_this.canvasFlag=true;
				...
				ctx.fillText('￥', 110, (355+w)*scaleH,60);
				...
				ctx.fillText(zpPrice, 120, (355+w)*scaleH,zpPriceW);
			
				ctx.beginPath();//开始一个新的路径
				ctx.setFontSize(10)//设置字体大小，默认10
				ctx.setFillStyle('#999')//文字颜色：默认黑色
				let orignPriceW = ctx.measureText(orignPrice).width//去掉市场价
				ctx.fillText(`￥${orignPrice}`, 120+zpPriceW+5, (355+w)*scaleH,orignPriceW); //5价格间距
				ctx.moveTo(120+zpPriceW+5,(351+w)*scaleH);//设置线条的起始路径坐标
				ctx.lineTo(120+zpPriceW+5+orignPriceW,(351+w)*scaleH);//设置线条的终点路径坐标
				...
				       src: code,
				       success(res) {
						ctx.drawImage(res.path,(_this.phoneW-70)/2,(370+w)*scaleH, 70,70)
						ctx.draw(true)
				      },
						fail(){
							_this.canvasFlag=true;
				...
				ctx.font = 'normal bold 14px sans-serif';
				ctx.fillText('德易物商城', (_this.phoneW-70)/2, (460+w)*scaleH,90);
				// 小程序的名称end
				// 长按/扫描识别查看商品
				ctx.setFontSize(14)
				ctx.setFillStyle('#ff5f33')//文字颜色：默认黑色
				ctx.font = 'normal 14px sans-serif';
				ctx.fillText('长按/扫描识别查看商品', (_this.phoneW-140)/2, (485+w)*scaleH,140);
				// 长按/扫描识别查看商品end
				//绘制保存按钮
				//ctx.save(); 

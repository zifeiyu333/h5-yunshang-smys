<template>
	<view class="ali">
		<view class="ali_top borders" v-if="state != 3 && orderAddress.areaname != undefined">
			<text class="ali_top_icon">收</text>
			<text class="a14">{{orderAddress.areaname}} {{orderAddress.detail}}</text>
		</view>
		<!-- ======="state": 0 ,1 ,2====== -->
			...
			<block v-for="(item,i) in datas" :key="i">
				<view class="State_Four">
					<view class="FourBox">
						...
						<text class="ali_top_icon" style="margin: 0 0;" v-if="i == datas.length -1">发</text>
					...
					<view class="remark">
						<!-- <text class="a14"  v-if="i == datas.length-1">已发货 {{item.status}}</text> -->
						<text class="a14">{{item.status}}</text>
...
<script>
	export default{
		props:["datas","state","ways","orderAddress"],
		data(){
			return{
		...
		methods: {
			...
			Type(item){
				console.info(item)
				console.info(item.includes('丰巢智能快递柜'))
				let Info;
				let Phone = /\d{11}/;
				if(Phone.test(item) && item.includes('正在派')){
					Info = true;

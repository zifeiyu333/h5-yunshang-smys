	...
	<view class="content">
		...
		<view class="listbox" v-for="(item, index) in list" :key="index">
			<view class="namebox">
				...
				<text class="number greencolor" v-if="item.ixType === 1">+￥{{item.amount}}</text>
				<text class="number redcolor" v-else>-￥{{item.amount}}</text>
			</view>
			<view class="daybox">
	...
	export default {
		...
		methods: {
			async loadData(type = 'add', loading) {
				...
				this.$api.request('id/api/apple/record/listAppleRecord', {
					page: this.page,
					limit: 10
				}).then(res => {

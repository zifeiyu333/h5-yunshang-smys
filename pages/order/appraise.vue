<template>
	<view class="page">
		<view v-for="(item, index) in orderDetail.orderGoodsList" :key="index">
			<view class="goods-box-single">
				<image class="goods-img" :src="item.goodsPic + '?x-oss-process'" mode="aspectFill"></image>
				<view class="right">
					<text class="title clamp">{{item.goodsName}}</text>
					<text class="attr-box">{{item.skuName}} x {{item.totalNum}}</text>
			...
			<view class='appraise-title appraise-star-view'>
				...
				<view class="appraise-star-view">
					<text class="appraise-star" v-for="(value,key) in stars" :key="key" :class="key < orderDetail.orderGoodsList[index].score ? 'active' : ''"
					 @tap="chooseStar(value,index)"></text>
				</view>
			</view>
			<view class="appraise-body">
				<textarea placeholder="请输入评价..." v-model="item.content" class="appraise-textare" />
				</view>
						...
						<view class="uni-uploader__files">
							<block v-for="(image,imgIndex) in item.imgs" :key="imgIndex">
								<view class="uni-uploader__file" style="position: relative;">
									<image class="uni-uploader__img" :src="image + '?x-oss-process'" @tap="previewImage(item.imgs)"></image>
									<view class="close-view" @click="close(item, index, imgIndex)">x</view>
								</view>
							</block>
							<view class="uni-uploader__input-box" v-show="item.imgs.length < 8">
    ...
    export default {
        ...
        onLoad(option) {
			...
			that.$api.request('mall/api/myOrder/orderDetail', {
				...
				that.orderDetail.orderGoodsList.forEach(item => {
					item.imgs = []
				})
				uni.hideLoading()
			})
        ...
        methods: {
            close(item, index, imgIndex){
				...
				this.orderDetail.orderGoodsList.splice(index, item)
            },
            ...
            chooseImg(item,index) { //选择图片
                ...
                that.$api.uploadImg((res => {
					item.imgs.push(res)
					that.orderDetail.orderGoodsList.splice(index, that.orderDetail.orderGoodsList[index])
				}))
            },
            chooseStar(e,index) { //点击评星
				const that = this
                that.orderDetail.orderGoodsList[index].score = e
				that.orderDetail.orderGoodsList.splice(index, that.orderDetail.orderGoodsList[index])
            },
            ...
            send() { //发送反馈
				...
				that.orderDetail.orderGoodsList.forEach(item => {
					...
					requestItems.push({
						...
						imgUrl: item.imgs.length > 0 ? item.imgs : []
					})
				
				})
				that.$api.request('mall/api/appraise/save', {
					orderId: that.orderDetail.orderId,
					appraiseList : requestItems

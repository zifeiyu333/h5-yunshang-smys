<template>
	<view class="content">
		<view class="row b-b" style="position: sticky;top: 0;">
			<text class="tiv">订单总数：{{termOrderCount.orderTotalNum}}</text>
			<text class="tiv">订单总金额：{{termOrderCount.orderTotalPrice}}</text>
		</view>
				...
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					...
					<view v-for="(item,index) in tabItem.orderList" :key="index" class="order-item">
						...
						<view class="price-box">
							<text class="time">购买人：{{item.buyNickName}}</text>
						</view>
						<view class="price-box">
							<text class="time">{{(item.addTime * 1000) | dateFormat}}</text>
							<view class="action-box">
								<text class="price" v-if="item.level === 1 && item.tuiPrice > 0">佣金：{{item.tuiPrice}}</text>
								<text class="price" v-if="item.level === 2 && item.tuiPrice > 0">佣金：{{item.tuiPrice}}</text>
								<text class="price" v-if="item.level === 3 && item.tuiPrice > 0">佣金：{{item.tuiPrice}}</text>
							</view>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				termOrderCount: {orderTotalNum: 0, orderTotalPrice: 0.00},
				userCode: '',
		...
		onLoad(options) {
			...
			this.$api.request('api/pk/order/termOrderCount', {
			}).then(res => {
				if (res.code === 200) {
					this.termOrderCount = res.data
				}
			}).catch(res => {
			})
			/**
		...
		methods: {
			...
			loadData(source) {
				...
				let orderList = that.$api.request('api/pk/order/termOrderList', {
			...
			weituoOrder(item) {
				...
				uni.showModal({
					...
					success : (e) => {
						if (e.confirm) {
							that.submiting = true
							that.$api.request('api/pk/order/weituoOrder', {
								weituoLogCode: item.weituoLogCode
							}, 'PUT').then(res => {
								that.submiting = false
								if (res.code === 200) {
			...
			extractOrder(item) {
				uni.redirectTo({
					url: '/pagesA/address/list?source=1'
				})
				// const that = this
				// uni.showModal({
				// 	title: '提取货物？',
